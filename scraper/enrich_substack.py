"""
AI Enrichment: Process raw Substack posts with Gemini API to generate:
- Summaries, key concepts, difficulty levels
- Cross-post connections, clusters, learning paths
- Quiz questions
Output: data/substack_enriched.json
"""
...
RAW_PATH = Path(__file__).parent.parent / "data" / "substack_raw.json"
OUTPUT = Path(__file__).parent.parent / "data" / "substack_enriched.json"
...
def enrich_single_post(post):
    ...
    subtitle_line = f"\nSubtitle: {post['subtitle']}" if post.get("subtitle") else ""

    prompt = f"""Analyze this Substack newsletter post by Kris Abdelmessih (Moontower) about finance, options, volatility, probability, or decision-making.

Title: {post['title']}{subtitle_line}
...
def generate_connections(posts_with_meta):
    ...
    prompt = f"""You have {len(posts_with_meta)} Substack newsletter posts by Kris Abdelmessih (Moontower) covering options, volatility, probability, decision-making, and related topics.
...
def generate_quizzes(posts, edges):
    ...
    for batch_start in range(0, len(posts), batch_size):
        ...
        prompt = f"""Generate quiz questions for these Substack newsletter posts by Kris Abdelmessih (Moontower) about options, volatility, and decision-making.
...
Make questions test genuine understanding, not just memorization."""
    ...
    for batch_start in range(0, len(edges), edge_batch_size):
        ...
        prompt = f"""Generate 1 connection quiz question per edge. These edges connect Substack posts about options, volatility, and decision-making.
...
def main():
    if not RAW_PATH.exists():
        print(f"Raw data not found at {RAW_PATH}. Run scrape_substack.py first.")
        sys.exit(1)
